for (x in 1:3){
	if(!dir.exists('./Scripts_model_R_granulosa')){
		dir.create('./Scripts_model_R_granulosa')
	}

  setwd("./Scripts_model_R_granulosa")
...
Rhinella_df <- read.table("./Species_information/Rhinella_granulosa_species_information.txt",h=T)
...
Simulations <- function(sim, df){
  ...
  input_fastsimcoal_spatial(working_folder = paste0("./Model",Model_number),
                    ...
                    species_demography = "./Species_information/Rhinella_granulosa_demography.txt",
                    species_assignment = df[,c(4,5)])
  } else {
...
mclapply(1:n_sim, function(sim,df) Simulations(sim,Rhinella_df), mc.cores=detectCores())
